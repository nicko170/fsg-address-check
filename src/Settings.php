...
class Settings
{
	public static function setup(): void
	{
		...
		add_action('admin_init', function () {
			register_setting('fsg_nbn_api_options', 'fsg_nbn_api_options', function ($input) {
				if (!preg_match('/^https:\/\//', $input['endpoint'])) {
					$input['endpoint'] = '';
				}
			...
			add_settings_field('fsg_nbn_api_setting_client_id', 'Client ID', fn() => self::settings_field('client_id', true), 'fsg_nbn_api', 'api_settings');
			add_settings_field('fsg_nbn_api_setting_username', 'Username', fn() => self::settings_field('username'), 'fsg_nbn_api', 'api_settings');
			add_settings_field('fsg_nbn_api_setting_password', 'Password', fn() => self::settings_field('password', true), 'fsg_nbn_api', 'api_settings');
		});

		add_action('wp_ajax_import_fsg_plans', function () {
			global $wpdb;
